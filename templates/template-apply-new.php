...
session_start();
/*
Template Name: Apply Now Another
*/
if(!empty($_SESSION['form']['question']) && $_SESSION['user']['LoggedIn'] == 1){		
	
	if($_SESSION['form']['question'] == 7){

		$current_step = 8;		

	}else if($_SESSION['form']['IncompleteStep'] == 'true' ){
		
		$current_step = $_SESSION['form']['question'];				

	}else if($_SESSION['form']['IncompleteStep'] == 'true' ){
		
		$current_step = $_SESSION['form']['question'] + 1;				
	}
...
<div id="main-content" >
	...
	<div id="apply-now-page">		
		...
		<div id="content" class="wrapper clearfix">									
			...
			<div id="questions" class="clearfix">
				...
				<form id="form-apply" action="<?php bloginfo('stylesheet_directory'); ?>/libraries/apply-now-handler.php" method="POST">
					...
					<?php if($_SESSION['user']['LoggedIn'] == 1) { ?>
						...
						<div class="user-info">
							...
							<a id="logout" href="<?php bloginfo('stylesheet_directory'); ?>/libraries/logout.php?url=<?php echo get_permalink(25); ?>">[ Logout ]</a>
						</div>
	...
	<!-- Lightboxes -->
	<div id="assist-window">
		
		<div class="form-title"><h4>Call for Assistance</h4></div>

		<div id="thank-you-assist">
			
			<p>Thank you for sending your information, a Borrower Satisfaction Representative will assist you in a moment.</p>

		</div>

		<form id="assist-form" action="<?php bloginfo('stylesheet_directory'); ?>/libraries/call-assist.php" method="POST">
			...
			<div class="row clearfix">
				...
				<input type="text" name="PhoneNumber" class="phone required" />
			...
			<div class="row clearfix">
				...
				<input type="text" name="Time" class="datepicker required" />
			...
			<div class="row">
				...
				<label>Do you want to finance the purchase of a home or refinance an existing loan ?</label>
				<label class="radio"><input type="radio" name="FinancType" value="Purchase" class="required" />Purchase</label>
				<label class="radio"><input type="radio" name="FinancType" value="Refinance" />Refinance</label>
	...
	<div id="save-window">
		...
		<!-- End Error Messages -->		
			...
			<form id="form-login" class="<?php if($_SESSION['user']['LoggedIn'] == 1) { echo 'hide'; } ?>" action="<?php bloginfo('stylesheet_directory'); ?>/libraries/login.php" method="POST">		
				<div class="row clearfix">
				
					<label>Email</label>
					<input type="text" class="email" name="UserEmail" />			
				...
				<div id="save-option" class="row" style="display:none;">
					
					<label><input type="checkbox" name="application" />Click here if you are a new user.</label>	

				</div>
				<!-- row -->

				<div class="row">
					<a id="submit-info" class="btn-green" href="#">Submit</a>
				</div>
			...
			<div id="form-logout" class="<?php if($_SESSION['user']['LoggedIn'] == 0) { echo 'hide'; } ?>">		

				You are already logged in, if you want to login with a different user please <a id="logout" href="<?php bloginfo('stylesheet_directory'); ?>/libraries/logout.php?url=<?php echo get_permalink(25); ?>">click here</a> to logout.
